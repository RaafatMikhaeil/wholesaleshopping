...
class Utility_Functions {
    ...
    public function htmlDiff($old, $new){
        ...
        foreach($diff as $k){
            if(is_array($k)){
                if(!empty($k['d']))
                    $ret['old'] = $ret['old'].'<del style="background-color:#ffcccc">'.implode(' ',$k['d']).'</del> ';
                else
                    $ret['old'] = $ret['old'].'';
                if(!empty($k['i']))
                    $ret['new'] = $ret['new'].'<ins style="background-color:#ccffcc">'.implode(' ',$k['i']).'</ins> ';
                else
                    $ret['new'] = $ret['new'].'';
            }else{
                $ret['old'] = $ret['old'].$k . ' ';
    ...
    public function getDiffSizeImage($picName, $size) {
        $ext = strtolower(substr($picName,-3));
        if( $ext == "peg" ) {
                $ext = "jpg";
        }

        $thumbName = preg_replace("/\.([a-zA-Z]{3,4})$/","_".$size.".".$ext,$picName);
        return $thumbName;
    }
}